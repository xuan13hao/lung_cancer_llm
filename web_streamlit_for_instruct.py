...
def generate_interactive(
    ...
    **kwargs,
    ...
    # Use the model's internal generation_config
    ...
    for _ in range(generation_config.max_new_tokens):
        model_inputs = model.prepare_inputs_for_generation(inputs['input_ids'], **kwargs)
        outputs = model(**model_inputs, return_dict=True)
        logits = outputs.logits[:, -1, :]
        next_token_scores = logits_processor(inputs['input_ids'], logits)
        next_token_scores = model._get_logits_warper(generation_config)(inputs['input_ids'], next_token_scores)
        next_tokens = torch.multinomial(nn.functional.softmax(next_token_scores, dim=-1), num_samples=1).squeeze(1)
        inputs['input_ids'] = torch.cat([inputs['input_ids'], next_tokens[:, None]], dim=-1)

        yield tokenizer.decode(inputs['input_ids'][0][input_length:], skip_special_tokens=True)

        if all(next_tokens == additional_eos_token_id):
            break
...
def prepare_generation_config(model):
    with st.sidebar:
        st.title('Hyperparameter Panel')
        system_prompt_content = st.text_area(
            'System Prompt',
            "You are a creative and knowledgeable AI assistant, named Llama3-Chinese. Your responses should be humorous yet precise.",
            height=200,
            key='system_prompt_content'
        )
        max_new_tokens = st.slider('Max Reply Length', 100, 8192, 660, step=8)
        top_p = st.slider('Top P', 0.0, 1.0, 0.8, step=0.01)
        temperature = st.slider('Temperature', 0.0, 1.0, 0.7, step=0.01)
        repetition_penalty = st.slider("Repetition Penalty", 1.0, 2.0, 1.07, step=0.01)
        st.button('Reset Chat', on_click=on_btn_click)

    # Modify the model's internal generation config
...
def main(model_name_or_path, adapter_name_or_path):
    st.title('Llama3-Chinese Chatbot')

    # Load the model
    model, tokenizer = load_model(model_name_or_path, adapter_name_or_path, load_in_4bit=False)
    generation_config = prepare_generation_config(model)
    ...
    if prompt := st.chat_input('Ask me anything!'):
        ...
        with st.chat_message('robot'):
            ...
            message_placeholder.markdown(response)
        st.session_state.messages.append({'role': 'robot', 'content': response})
