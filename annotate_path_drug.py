...
def get_relation_direction(source, relation, target):
    ...
    """
    full_prompt = (
        f"I am a helpful AI Lung Cancer Oncology Assistant. Please Provide concise answer. Determine the relation direction between '{source}' and '{target}' "
        f"with relation '{relation}'. Just return one of these numbers: "
        "suppressed (0), active (1), no relation (2), or not sure (3). Do not include any explanation, chain-of-thought, or intermediate reasoning. Final Answer is:"
    )
...
relations_df.to_csv('drug_protein_direction.csv', index=False)

print("The relations with their directions have been written to 'drug_protein_direction.csv'.")
